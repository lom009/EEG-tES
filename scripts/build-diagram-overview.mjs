...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .view { height: 100%; min-height: 0; display: grid; gap: 12px; }
    .view[hidden] { display: none !important; }
    .view.single { grid-template-columns: minmax(0,1fr); }
    .view.course-reading,
    .view.analysis-reading,
    .view.integration-reading,
    .view.consensus-reading { grid-template-columns: clamp(360px, 34vw, 520px) minmax(0,1fr); }
    .note-panel {
      min-width: 0;
      min-height: 0;
      overflow: auto;
    ...
    .note-panel::-webkit-scrollbar { width: 10px; }
    .note-panel::-webkit-scrollbar-track { background: #edf2f7; border-radius: 999px; }
    .note-panel::-webkit-scrollbar-thumb { background: #9fb3c8; border: 2px solid #edf2f7; border-radius: 999px; }
    .note-panel::-webkit-scrollbar-thumb:hover { background: #7891aa; }
    ...
    @media (max-width: 980px) {
      ...
      .view.analysis-reading,
      .view.integration-reading,
      .view.consensus-reading { grid-template-columns: 1fr; grid-template-rows: minmax(260px, .85fr) minmax(420px, 1.15fr); overflow: auto; }
    }
  </style>
</head>
<body>
  ...
  <script>
    ...
    function createView(mode) {
      ...
      if (mode === 'consensus') {
        view.className = 'view consensus-reading';
        view.appendChild(createConsensusNotes());
        view.appendChild(createConsensusBoard());
      } else if (mode === 'course' || mode === 'analysis' || mode === 'integration') {
        view.className = 'view ' + mode + '-reading';
        ...
        view.appendChild(createPane(mode));
      } else {
        view.className = 'view single';
        view.appendChild(createPane(mode));
      }
      view.hidden = true;
      viewCache.set(mode, view);
    ...
    function render(mode) {
      ...
      const nextView = viewCache.get(mode) || createView(mode);
      viewCache.forEach((view, key) => { view.hidden = key !== mode; });
